<script>
  ...
  AFRAME.registerComponent('moveup', {
    ...
    update: function (oldData) {
      ...
      if (oldData.id && data.id !== oldData.id) {
        el.removeEventListener('grab-start', this.eventHandlerFn);
      }

      if (data.id) {
        el.addEventListener('grab-start', this.eventHandlerFn);
      }
    },
	...
	AFRAME.registerComponent('movedown', {
    schema: {
      id: {default: 'none'}
    },
